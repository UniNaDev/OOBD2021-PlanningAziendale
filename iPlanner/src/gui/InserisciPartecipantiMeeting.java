...
public class InserisciPartecipantiMeeting extends JFrame {
	...
	//Creazione frame
	//---------------------------------------------
	public InserisciPartecipantiMeeting(ControllerMeeting theController,Meeting meetingSelezionato, int codiceMeeting) {
		...
		try {
			listmodel.addAll(theController.ottieniInvitati(codiceMeeting));
		...
		eliminaPartecipanteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(partecipantiList.isSelectionEmpty()) {
					...
					try {
						theController.eliminaPartecipante((Dipendente)partecipantiList.getSelectedValue(),codiceMeeting);
		...
		inserisciPartecipanteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(dipendenteTable.getSelectedRow()==-1) {
					...
					try {
						theController.inserisciPartecipante(dipendenteTable.getValueAt(row, 0).toString(), codiceMeeting);
						JOptionPane.showMessageDialog(null, "Invitato inserito correttamente");
						dataModelDipendente.setDipendenteTabella(theController.ottieniDipendenti());
						listmodel.removeAllElements();
						listmodel.addAll(theController.ottieniInvitati(codiceMeeting));
					} catch (SQLException e1) {
		...
		try {
			dataModelDipendente.setDipendenteTabella(theController.ottieniDipendenti());	//setta il modello di dati della tabella
		} catch (SQLException e1) {
			JOptionPane.showMessageDialog(null, e1.getMessage());
		}
		//Click sulla tabella
		dipendenteTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					skillModel.addAll(theController.ottieniSkillDipendente(dipendenteTable.getValueAt(row, 0).toString()));
